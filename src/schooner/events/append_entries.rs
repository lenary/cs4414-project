...
mod test {
    use serialize::{json, Decodable};
    use super::super::super::consistent_log::LogEntry;
    
    use super::{AppendEntriesReq, AppendEntriesRes};

    #[test]
    fn test_json_encode_of_AppendEntriesRes() {
        let aeresp = super::AppendEntriesRes{success: true,
                                                  term: 33,
                                                  idx: 22,
                                                  commit_idx: 5,
                                                  peer_id: 2};
    ...
    #[test]
    fn test_json_decode_of_AppendEntriesRes() {
        ...
        let aeresp: super::AppendEntriesRes = Decodable::decode(&mut decoder).unwrap();
    ...
    #[test]
    fn test_json_decode_of_AppendEntriesRes_via_api() {
        let jstr = ~"{\"success\":true,\"term\":777,\"idx\":99,\"commit_idx\":33,\"peer_id\":2}";

        let result = AppendEntriesRes::decode(jstr);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_json_encode_of_AppendEntriesReq() {
        ...
        let aereq = super::AppendEntriesReq{term: 66,
                                                prev_log_idx: 13,
                                                prev_log_term: 1,
                                                commit_idx: 88,
                                                leader_id: 32,
    ...
    #[test]
    fn test_json_decode_of_AppendEntriesReq() {
        ...
        let mut decoder = json::Decoder::new(jobj.unwrap());
        let appendreq: super::AppendEntriesReq = Decodable::decode(&mut decoder).unwrap();
