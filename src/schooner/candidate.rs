...
pub trait Candidate {
    ...
    fn candidate_append_entries_req(&mut self, req: (AppendEntriesReq, Sender<AppendEntriesRes>)) -> RaftStateTransition;
    fn candidate_append_entries_res(&mut self, res: AppendEntriesRes) -> RaftStateTransition;

    fn candidate_vote_req(&mut self, req: (VoteReq, Sender<VoteRes>)) -> RaftStateTransition;
